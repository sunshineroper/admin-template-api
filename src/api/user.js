...
userRouter.sGet('获取用户列表带分页', '/getUserList', userRouter.permission('获取用户列表带分页'), groupRequired, async (ctx) => {
  const v = await new PageValidator().validate(ctx)
  const userList = await UserController.getUserList(v, ctx)
  ctx.json(userList)
})

userRouter.sPost('新增用户', '/addUser', userRouter.permission('新增用户'), groupRequired, async (ctx, next) => {
  await next()
  const v = await new AddUserValidator().validate(ctx)
  return await UserController.addOrEditUser(v, ctx)
}, log('新增用户'))

userRouter.sGet('获取用户信息', '/getUserInfo', userRouter.permission('获取用户信息', false), loginRequired, async (ctx, next) => {
  const user = ctx.currentUser
  ctx.json(user)
  await next()
}, log('获取用户信息'))

userRouter.sPut('修改用户', '/editUser/:id', userRouter.permission('修改用户'), groupRequired, async (ctx, next) => {
  const v = await new PositiveIdValidator().validate(ctx)
  await next()
  return await UserController.addOrEditUser(v, ctx)
}, log('修改用户'))

userRouter.sDelete('删除用户', '/deleteUser/:id', userRouter.permission('删除用户'), groupRequired, async (ctx, next) => {
  const v = await new PositiveIdValidator().validate(ctx)
